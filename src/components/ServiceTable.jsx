...
const ServiceTable = () => {
    ...
    const { services, editService, deleteService, addToCart } = useServices();
        ...
        <>
            <div className='container '>
                <div className='flex flex-col items-center w-full'>
                    <h1 className='m-5 text-xl font-bold uppercase'>Listado de Servicios</h1>
                    <GloablFilter filter={globalFilter} setFilter={setGlobalFilter} text="Buscar Servicios" />
                </div>
                                    ...
                                    <tr className="bg-[#130e5f] hover:bg-[]" {...row.getRowProps()}>
                                        {row.cells.map(cell => {
                                            if (cell.column.Header === 'Edicion') {
                                                return (
                                                    <td key={cell.value} className="flex flex-col justify-center items-center">
                                                        <a href="#" className="font-medium text-blue-500  hover:underline " onClick={() => addToCart(cell.value)}>Agregar</a>
                                                        <a href="#" className="font-medium text-green-500 hover:underline " onClick={() => { editService(cell.value); navigate("/admin/services-form") }}>Editar</a>
                                                        <a href="#" className="font-medium text-red-500  hover:underline " onClick={() => deleteService(cell.value)}>Eliminar</a>
                                                    </td>
                                                )
                                            }
                                            return (
